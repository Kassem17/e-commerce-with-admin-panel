    ...
    addToCart(
      { productId: product._id, quantity },
      {
        ...
        onError: (error: any) => {
          Alert.alert(
            "Error",
            error?.response?.data?.error || `Failed to added to cart`
          );
        },
      }
    );
  ...
  return (
    <SafeScreen>
      ...
      <View className="absolute top-0 left-0 right-0 z-10 px-6 pt-20 pb-4 flex-row items-center justify-between">
        <TouchableOpacity
          className="rounded-full bg-surface/50 backdrop-blur-xl w-12 h-12 items-center justify-center"
          activeOpacity={0.7}
          onPress={() => router.back()}
        >
      ...
      {/* Bottom Action Bar */}
      <View className="absolute bottom-0 left-0 right-0 bg-background/95 backdrop-blur-xl border-t border-surface px-6 py-4 pb-10">
