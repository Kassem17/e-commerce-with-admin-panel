...
import { capitalizeFirstLetter, getStatusColor, formatDate } from "@/lib/utils";
...
const orders = () => {
  ...
  const handleOpenRating = (order: Order) => {
    ...
    // initialize ratings for all products to 0 - resetting the state
    const initialRating: { [key: string]: number } = {};
    order.orderItems.forEach((item) => {
      const productId = item.product._id;
      initialRating[productId] = 0;
    });
    setProductRatings(initialRating);
  };
  ...
  return (
    <SafeScreen>
      ...
      {/* CREATE REVIEW SUCCESS MODAL */}
      <Modal
        visible={reviewSuccessVisible}
        transparent
        animationType="fade"
        onRequestClose={() => setReviewSuccessVisible(false)}
      >
        <View className="flex-1 justify-center items-center bg-black/50">
          <View className="bg-white rounded-2xl p-6 w-11/12 max-w-md items-center">
            ...
            <Text className="text-xl font-bold mt-4">Reviewed Successful!</Text>
